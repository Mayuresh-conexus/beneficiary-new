...
@section('header', 'Edit Role: ' . $role->display_name)
@section('subheader', 'Update role permissions and details')
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-8">
        <form method="POST" action="{{ route('roles.update', $role) }}">
            ...
            <div class="space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" value="{{ $role->name }}" disabled class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-slate-400 font-mono outline-none cursor-not-allowed"/>
                        <p class="text-[10px] text-slate-400 mt-1">Internal names cannot be changed.</p>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Display Name *</label>
                        <input type="text" name="display_name" value="{{ old('display_name', $role->display_name) }}" required class="w-full px-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none" placeholder="e.g. Project Manager"/>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            <label class="group flex items-start gap-3 p-4 rounded-xl border border-slate-100 hover:border-primary/20 hover:bg-primary/[0.02] cursor-pointer transition-all">
                                <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" {{ in_array($permission->name, $rolePermissions) ? 'checked' : '' }} class="mt-1 w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary"/>
                                <div>
            ...
            <div class="mt-8 flex gap-3 pt-6 border-t border-slate-100">
                <button type="submit" class="bg-primary text-white px-8 py-3 rounded-lg text-sm font-semibold hover:bg-primary/90 transition-all shadow-lg shadow-primary/20">Update Role</button>
